 ...
 * BCryptPasswordEncoder(); // 비밀번호 해싱 기능만 사용 }
 * 
 * // ✅ 핵심: firewall 설정 추가
 * 
 * @Bean public HttpFirewall allowDoubleSlashFirewall() { StrictHttpFirewall
 ...
 * .anyRequest().permitAll() // 모든 요청 허용 (로그인 필요 없음) ) .csrf(csrf ->
 * csrf.disable()) // CSRF 보호 비활성화 (필요 시 설정 가능) .formLogin(form ->
 * form.disable()) // 기본 로그인 페이지 비활성화 .httpBasic(basic -> basic.disable()); //
 * HTTP Basic 인증 비활성화
 * 
 * return http.build(); } }
 */