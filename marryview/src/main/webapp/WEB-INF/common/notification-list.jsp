<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    <%@ taglib prefix="c" uri="jakarta.tags.core" %>
        ...
        <body class="${sessionScope.sessionRole eq 'ADMIN' ? 'admin-notification-page' : ''}">
            ...
            <div id="app" v-cloak data-api-base="${pageContext.request.contextPath}/api/notification"
                data-context-path="${pageContext.request.contextPath}" data-session-role="${sessionScope.sessionRole}">
                <aside class="side-visual side-visual--left" aria-hidden="true">
                    <video class="side-video" id="weddingSideVideo"
                        data-src="${pageContext.request.contextPath}/img/weddinggirlmovie.mp4"
                        data-poster="${pageContext.request.contextPath}/img/weddingPic.png" autoplay muted loop
                ...
                <aside class="product-ad-rail" aria-label="메리뷰 입점 업체 추천 상품">
                    <article class="product-ad-card">
                        ...
                        <div class="product-ad-body">
                            <div class="product-ad-visual" :class="currentProductAd.tone" aria-hidden="true">
                                <img v-if="currentProductAd.imageUrl" :src="currentProductAd.imageUrl"
                                    :alt="currentProductAd.name" class="product-ad-img"
                                    @error="currentProductAd.imageUrl = ''">
                                <span v-else>{{ currentProductAd.symbol }}</span>
                            </div>
                            <div>
                        ...
                        <div class="product-ad-bottom">
                            <div class="product-ad-dots" aria-hidden="true">
                                <span v-for="(_, index) in productAds" :key="index" class="product-ad-dot"
                                    :class="{ active: index === productAdIndex }"></span>
                            </div>
                            <div class="product-ad-actions">
                                <button type="button" class="product-ad-arrow" @click="moveProductAd(-1)"
                                    aria-label="이전 추천 상품">‹</button>
                                <button type="button" class="product-ad-arrow" @click="moveProductAd(1)"
                                    aria-label="다음 추천 상품">›</button>
                                <button type="button" class="product-ad-link"
                                    @click="goProduct(currentProductAd)">보러가기</button>
                            </div>
                        </div>
                    </article>
                </aside>
                <aside class="wedding-celebration-rail" aria-label="메리뷰 회원 결혼 소식">
                    <article class="celebration-card">
                        ...
                        <h2 class="celebration-names">{{ currentWeddingNews.names }}님</h2>
                        ...
                        <div class="celebration-bottom" :class="{ 'is-mine': currentWeddingNews.isMine }">{{
                            currentWeddingNews.bottom }}</div>
                    </article>
                    <div class="celebration-nav" aria-label="결혼 소식 이동">
                        <button type="button" class="celebration-arrow" @click="moveCelebration(-1)"
                            aria-label="이전 결혼 소식">‹</button>
                        <span class="celebration-position">{{ celebrationIndex + 1 }} / {{ weddingNews.length }}</span>
                        <button type="button" class="celebration-arrow" @click="moveCelebration(1)"
                            aria-label="다음 결혼 소식">›</button>
                    </div>
                    ...
                    <section class="popular-feed" aria-labelledby="popularFeedTitle">
                        ...
                        <div class="popular-tabs" role="tablist" aria-label="인기 콘텐츠 종류">
                            <button type="button" class="popular-tab" :class="{ active: popularType === 'post' }"
                                @click="popularType = 'post'" role="tab"
                                :aria-selected="popularType === 'post'">인기글</button>
                            <button type="button" class="popular-tab" :class="{ active: popularType === 'review' }"
                                @click="popularType = 'review'" role="tab"
                                :aria-selected="popularType === 'review'">인기리뷰</button>
                        </div>
                        <ol class="popular-list">
                            <li v-for="(item, index) in currentPopularItems" :key="item.title">
                                <button type="button" class="popular-item" @click="goPopularItem(item)"
                                    :aria-label="(index + 1) + '위 ' + item.title">
                                    <span class="popular-rank">{{ index + 1 }}</span>
                                    <span>
                                        <span class="popular-item-title">{{ item.title }}</span>
                                        <span class="popular-item-meta"><span>조회 {{ item.views }}</span><span>♥ {{
                                                item.likes }}</span></span>
                                    </span>
                                </button>
                            </li>
                        </ol>
                    ...
                    <section class="notification-card" aria-labelledby="notificationHeading">
                        <div class="toolbar">
                            ...
                            <div class="toolbar-actions">
                                <button type="button" class="btn btn-ghost" @click="refreshAll(true)"
                                    :disabled="refreshing">
                                    <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                                ...
                                <button type="button" class="btn btn-primary" @click="readAll"
                                    :disabled="refreshing || unreadCount === 0">모두 읽음</button>
                            </div>
                        </div>
                        ...
                        <div class="filter-row">
                            <div class="filters" role="tablist" aria-label="알림 필터">
                                <button v-for="filter in filters" :key="filter.value" type="button" class="filter-btn"
                                    ...
                                    @click="changeFilter(filter.value)" role="tab"
                                    :aria-selected="currentFilter === filter.value">{{ filter.label }}</button>
                            </div>
                        ...
                        <div class="notification-list" aria-live="polite">
                            <div v-if="loading" class="state-view">
                                <div>
                                    <div class="loading-dots" aria-label="알림 불러오는 중">
                            ...
                            <div v-else-if="errorMessage" class="state-view">
                                <div>
                                    ...
                                    <h3 class="state-title">알림을 불러오지 못했어요</h3>
                                    <p class="state-copy">{{ errorMessage }}</p>
                                </div>
                            </div>
                            <div v-else-if="filteredNotifications.length === 0" class="state-view">
                                <div>
                                    <div class="state-icon">{{ emptyState.icon }}</div>
                                    <h3 class="state-title">{{ emptyState.title }}</h3>
                                    <p class="state-copy">{{ emptyState.copy }}</p>
                                </div>
                            </div>
                            <template v-else>
                                <button v-for="item in pagedNotifications" :key="item.notificationNo" type="button"
                                    class="notification-page-item" :class="{ unread: item.isRead === 'N' }"
                                    @click="readNotification(item)"
                                    :aria-label="(item.isRead === 'N' ? '읽지 않은 알림: ' : '') + (item.content || '알림 내용 없음')">
                                    <span class="type-icon" aria-hidden="true">{{ item.isRead === 'N' ?
                                        typeIcon(item.notificationType) : '✓' }}</span>
                                    <span>
                                        <span class="item-head"><span class="item-type">{{
                                                typeLabel(item.notificationType) }}</span><span
                                                v-if="item.isRead === 'N'" class="new-dot"
                                                aria-label="새 알림"></span></span>
                        ...
                        <div v-if="filteredNotifications.length > pageSize" class="notification-pagination">
                            <button type="button" class="page-btn" @click="movePage(currentPage - 1)"
                                :disabled="currentPage === 1">
                                이전
                            </button>

                            <button v-for="page in totalPages" :key="page" type="button" class="page-number"
                                :class="{ active: currentPage === page }" @click="movePage(page)">
                                {{ page }}
                            </button>
                ...
                <div class="toast" :class="{ show: toastVisible }" role="status" aria-live="polite">{{ toastMessage }}
                </div>
